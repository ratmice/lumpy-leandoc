...
const LEAN3_SYNTAX: &str = include_str!("../assets/Lean3.sublime-syntax");
...
pub fn handle_event<'a>(
    ...
    match event_tuple.0 {
        ...
        cmark::Event::Html(html_str) => Err(ErrorKind::MarkdownLatexConversion(format!(
            "unsupported html: {}",
            html_str
        ))
        .into()),
        cmark::Event::InlineHtml(html_str) => Err(errors::AppError::App(
            ErrorKind::MarkdownLatexConversion(format!("unsupported inline html: {}", html_str)),
        )
        .into()),
        ...
        cmark::Event::TaskListMarker(_flag) => Err(ErrorKind::MarkdownLatexConversion(
        ...
        _ => Err(ErrorKind::MarkdownLatexConversion(format!("unsupported markdown tag")).into()),
    }
}
